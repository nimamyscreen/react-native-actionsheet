...
class ActionSheet extends Component {
  ...
  _calculateHeight(props) {
    ...
    let height = BUTTON_H * count + CANCEL_MARGIN+15
    ...
    if (cancelButtonIndex > -1 && options[cancelButtonIndex]) {
      return (
        <TouchableHighlight
          ...
          style={[btnStyle.wrapper, {backgroundColor:'white',height: 55,marginBottom:15,borderWidth:0,borderBottomLeftRadius:4,borderBottomRightRadius:4,borderTopWidth:0,borderColor:'gray'}]}
          onPress={this._cancel}
        >
          <Text style={[btnStyle.title, {fontWeight: '700', color: tintColor}]}>{options[cancelButtonIndex]}</Text>
          {/*<Ionicons name='ios-close' color="" size={50}/>*/}
        </TouchableHighlight>
      )
    } else {
      return null
